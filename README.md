...
- DB커넥션 객체를 여러 요청이 공유할 시, 오류가 발생하여 롤백(rollback) 필요할 때, 다른 요청들까지 영향을 받는다.
...
---
#### 40. 프런트 컨트롤러 도입
- 현재 서블릿에서는 VO객체 준비, 뷰 컴포넌트로의 위임, 오류처리, 페이지 작업 등을 모두 담당하고 있다.
- VO객체 준비, 뷰 컴포넌트로의 위임, 오류처리 등과 같은 공통 작업들은 프런트 컨트롤러가 담당하게하고, 페이지 컨트롤러는 페이지만을 위한 작업을 수행하도록 변경한다.

---
#### 41. 페이지 컨트롤러 생성
- 서블릿 인터페이스를 구현했던 페이지 서블릿들은 이제 VO객체 준비, 뷰 컴포넌트로의 위임, 오류처리 등과 같은 공통 작업을 프런트 컨트롤러에서 처리하므로 더 이상 서블릿일 필요가 없으므로 서블릿 API를 제거하고 일반 클래스로 만들었다.
...
---
#### 43. 리플랙션 API를 이용하여 프런트 컨트롤러 개선
- 프런트 컨트롤러는 아직까지도 페이지 컨트롤러 추가할 때마다 코드를 변경해야 한다.
- 특히 매개변수 값을 받아서 VO 객체를 생성하는 부분에 많은 손이 간다.
- 페이지 컨트롤러에게 어떤 데이터가 필요한지를 물어보고 VO객체를 자동으로 만들게 한다.
- 리플랙션 API를 활용하면 편리하게 작성할 수 있다.
...
---
#### 45. 프로퍼티 파일을 이용한 객체 관리
- 리플랙션 API를 통해 프런트 컨트롤러를 개선하였다.
- 하지만 ContextLoaderListener에서는 페이지 컨트롤러 객체를 생성하기 때문에 아직까지 손을 대야한다.
- 소스 코드로부터 별도로 프로퍼티 파일을 생성하여 객체 관리를 할 수 있도록 수정했다.

---
#### 46. 애노테이션을 이용한 객체 관리
- 프로퍼트 파일을 통해 별도로 분리하여 손이 덜가지만 그대로 추가 될때마다 손을 봐야한다.
- 생성한 소스 코드 위에 애노테이션 붙이는 것만으로도 프로퍼티 파일의 내용에는 손댈 필요없이 객체르 관리할 수 있다.
